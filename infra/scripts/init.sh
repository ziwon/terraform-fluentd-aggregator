...
create_ecr() {
	...
	info "ECR Repo:" "$(aws ecr describe-repositories --repository-names "${APP_NAME}" | jq -r '.repositories[0].repositoryUri')"
}
...
set_workspace() {
	...
	info "Current workspace is: $workspace"
}
...
if has_credentials; then
	...
	create_ecr
	init_backend
	set_workspace "$PROJECT_NAME-$ENV"
fi
